...
export default defineConfig(({ mode }) => {
  if (['value', 'config', 'event'].includes(mode)) {
    ...
    return {
      ...
      build: {
        ...
        minify: false,
        target: 'esnext',
        outDir: `../ruomu-page/public/entry/${mode}`,
  ...
  if (['page', 'designer'].includes(mode)) {
    const base = `/runtime/${mode}`;
    const outDir = path.resolve(process.cwd(), `../ruomu-page/public/runtime/${mode}`);
    return {
      plugins: [
        vue(),
        vueJsx(),
